...
class details_controller
{
	...
	*/
	public function details($uid, $sort_key, $sort_dir, $page)
	{
		...
		if ($this->user->data['user_id'] != $uid && !$this->auth->acl_get('u_rs_view'))
		{
	...
	* @access public
	*/
	public function detailsgiven($uid, $sort_key, $sort_dir, $page)
	{
		$this->user->add_lang_ext('pico/reputation', array('reputation_system', 'reputation_rating'));
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$this->template->assign_block_vars('reputation', array(
				'ID'			=> $row['reputation_id'],
				'USERNAME'		=> get_username_string('full', $row['user_id_to'], $row['username'], $row['user_colour']),
				'ACTION'		=> $this->user->lang('RS_' . strtoupper($row['reputation_type_name']) . '_RATING'),
				'AVATAR'		=> phpbb_get_user_avatar($row),
				'TIME'			=> $this->user->format_date($row['reputation_time']),
				'COMMENT'		=> $row['reputation_comment'],
		...
		$this->pagination->generate_template_pagination(
			array(
				'routes'	=> 'reputation_details_given_controller',
				'params'	=> array(
					'uid'		=> $uid,
					'sort_key'	=> $sort_key,
					'sort_dir'	=> $sort_dir,
		...
		$this->template->assign_vars(array(
			...
			'U_SORT_USERNAME'	=> $this->helper->route('reputation_details_given_controller', array('uid' => $uid, 'sort_key' => 'username', 'sort_dir' => ($sort_key == 'username' && $sort_dir == 'asc') ? 'dsc' : 'asc',)),
			'U_SORT_TIME'		=> $this->helper->route('reputation_details_given_controller', array('uid' => $uid, 'sort_key' => 'time', 'sort_dir' => ($sort_key == 'time' && $sort_dir == 'asc') ? 'dsc' : 'asc',)),
			'U_SORT_POINT'		=> $this->helper->route('reputation_details_given_controller', array('uid' => $uid, 'sort_key' => 'point', 'sort_dir' => ($sort_key == 'point' && $sort_dir == 'asc') ? 'dsc' : 'asc',)),
			'U_SORT_ACTION'		=> $this->helper->route('reputation_details_given_controller', array('uid' => $uid, 'sort_key' => 'action', 'sort_dir' => ($sort_key == 'action' && $sort_dir == 'asc') ? 'dsc' : 'asc',)),
		...
		return $this->helper->render('detailsgiven.html', $this->user->lang('RS_DETAILS'));
	}
	...
	*/
	public function postdetails($post_id, $sort_key, $sort_dir)
	{
		...
		$sql_array = array(
			'SELECT'	=> 'p.forum_id, p.poster_id, p.post_subject, u.username, u.user_colour, u.user_id',
			'FROM'		=> array(
				POSTS_TABLE => 'p',
				USERS_TABLE => 'u'
			),
		...
		if ($this->user->data['user_id'] != $post_row['user_id'] && !$this->auth->acl_get('u_rs_view'))
		{
